...
def load_image(filename, is_mask=False):
    ...
    if is_mask:
        ...
        # Threshold the image: values >= 128 become 1, others become 0
        img = (img >= 128).astype(np.uint8)

        img = Image.fromarray(img)  # Convert back to PIL Image for consistency
    return img
...
class BasicDataset(Dataset):
    def __init__(self, images_dir: str, mask_dir: str, scale: float = 1.0, mask_suffix: str = '', transform=None):
        ...
        self.transform = transform  # transform parameter
